...
from pickle_court_reference import CourtReference
...
class CourtDetector:
    ...
    def detect(self, frame, verbose=0):
        ...
        court_warp_matrix, game_warp_matrix, self.court_score = self._find_homography(
    ...
    def _detect_lines(self, gray):
        ...
        lines = cv2.HoughLinesP(
            gray, 1, np.pi / 180, 80, minLineLength=minLineLength, maxLineGap=maxLineGap
        )
    ...
    def _classify_lines(self, lines):
        ...
        for line in horizontal:
            x1, y1, x2, y2 = line
            if (
                lowest_vertical_y > y1 > highest_vertical_y
                and lowest_vertical_y > y1 > highest_vertical_y
            ):
    ...
    def _find_homography(self, horizontal_lines, vertical_lines):
        ...
        for horizontal_pair in list(combinations(horizontal_lines, 2)):
            for vertical_pair in list(combinations(vertical_lines, 2)):
                ...
                i1 = line_intersection(
                    (tuple(h1[:2]), tuple(h1[2:])), (tuple(v1[0:2]), tuple(v1[2:]))
                )
                i2 = line_intersection(
                    (tuple(h1[:2]), tuple(h1[2:])), (tuple(v2[0:2]), tuple(v2[2:]))
                )
                i3 = line_intersection(
                    (tuple(h2[:2]), tuple(h2[2:])), (tuple(v1[0:2]), tuple(v1[2:]))
                )
                i4 = line_intersection(
                    (tuple(h2[:2]), tuple(h2[2:])), (tuple(v2[0:2]), tuple(v2[2:]))
                )
    ...
    def _get_confi_score(self, matrix):
        ...
        court = cv2.warpPerspective(
            self.court_reference.court, matrix, self.frame.shape[1::-1]
        )
        court[court > 0] = 1
        gray = self.gray.copy()
        gray[gray > 0] = 1
    ...
    def add_court_overlay(
        self, frame, homography=None, overlay_color=(255, 255, 255), frame_num=-1
    ):
        """
        ...
        if (
            homography is None
            and len(self.court_warp_matrix) > 0
            and frame_num < len(self.court_warp_matrix)
        ):
    ...
    def find_lines_location(self):
        ...
        self.lines = cv2.perspectiveTransform(
            self.p, self.court_warp_matrix[-1]
        ).reshape(-1)
    ...
    def track_court(self, frame):
        ...
        if self.frame_points is None:
            conf_points = np.array(
                self.court_reference.court_conf[self.best_conf], dtype=np.float32
            ).reshape((-1, 1, 2))
            self.frame_points = (
                cv2.perspectiveTransform(conf_points, self.court_warp_matrix[-1])
                .squeeze()
                .round()
            )
        ...
        for line in lines:
            ...
            points_on_line = np.linspace(line[0], line[1], 102)[
                1:-1
            ]  # 100 samples on the line
            p1 = None
            p2 = None
            if (
                line[0][0] > self.v_width
                or line[0][0] < 0
                or line[0][1] > self.v_height
                or line[0][1] < 0
            ):
                for p in points_on_line:
                    if 0 < p[0] < self.v_width and 0 < p[1] < self.v_height:
                        ...
                        break
            if (
                line[1][0] > self.v_width
                or line[1][0] < 0
                or line[1][1] > self.v_height
                or line[1][1] < 0
            ):
            ...
            if p1 is not None or p2 is not None:
                ...
                points_on_line = np.linspace(
                    p1 if p1 is not None else line[0],
                    p2 if p2 is not None else line[1],
                    102,
                )[1:-1]
            ...
            for p in points_on_line:
                ...
                bottom_y, bottom_x = min(p[1] + self.dist, self.v_height), min(
                    p[0] + self.dist, self.v_width
                )
            ...
            if len(new_points) < 50:
                ...
                if self.dist > 20:
                    ...
                    conf_points = np.array(
                        self.court_reference.court_conf[self.best_conf],
                        dtype=np.float32,
                    ).reshape((-1, 1, 2))
                    self.frame_points = (
                        cv2.perspectiveTransform(
                            conf_points, self.court_warp_matrix[-1]
                        )
                        .squeeze()
                        .round()
                    )
        ...
        matrix, _ = cv2.findHomography(
            np.float32(self.court_reference.court_conf[self.best_conf]),
            intersections,
            method=0,
        )
        inv_matrix = cv2.invert(matrix)[1]
        ...
        self.new_lines = cv2.perspectiveTransform(
            self.pts, self.court_warp_matrix[-1]
        ).reshape(-1)
